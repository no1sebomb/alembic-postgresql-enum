...
* [Module level enums](#module-level-enums)
...
## Module level enums<a id="module-level-enums"></a>

***Can be enabled with `module_level_enums` configuration flag***

By default every enum is repeated in every operation that uses it.
With `module_level_enums` turned on, enums that are created or dropped inside migration
are defined once as module level variables and then reused by both upgrade and downgrade functions.
...
# ### enums auto generated by alembic-postgresql-enum ###
myenum = postgresql.ENUM('one', 'two', 'three', name='myenum', create_type=False)
# ### end enums ###


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    myenum.create(op.get_bind())
    op.add_column('example_table', sa.Column('enum_field', myenum, nullable=False))
    # ### end Alembic commands ###
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_column('example_table', 'enum_field')
    myenum.drop(op.get_bind())
    # ### end Alembic commands ###
```

### Where enums are defined

By default definitions are rendered into `${imports}` section of your `script.py.mako` template,
so they are placed right after imports and above revision identifiers.

To choose the place for them yourself add `${enums if enums else ""}` to the template:
...
Then enums are defined there and not among the imports.
Migrations that define no enums leave that section empty,
so the template keeps working for `alembic revision` without `--autogenerate` as well.

Definitions are indented to match the line that uses `${enums}`,
so they can be placed inside `upgrade` and `downgrade` functions as well:
...
Keep in mind that in this case every function gets its own copy of the definitions.

Note that migrations are generated the usual way only if `${imports}` section is present in the template,
because definitions need `from sqlalchemy.dialects import postgresql` import.

Only enums that are created or dropped get a variable, `op.sync_enum_values` calls are not affected,
because they operate on enum values and not on enum types.

Variable name is the name of the enum, prefixed with schema name for enums outside of default schema.
Names that are not valid python identifiers are converted into ones,
and duplicates get a numeric suffix. To choose names yourself use `enum_variable_name`:
...
alembic_postgresql_enum.set_configuration(
    alembic_postgresql_enum.Config(
        module_level_enums=True,
        # schema is None for enums of default schema
        enum_variable_name=lambda schema, name: f'{name}_type',
    )
)
```
...
- `module_level_enums` (`False` by default) - feature flag that can be turned on to define enums that are created 
or dropped inside migration as module level variables, so they are not repeated in every operation. 
Definitions are placed into `${enums}` section of migration template if it has one, otherwise among the imports. 
See [module level enums](#module-level-enums).

- `enum_variable_name` (enum name prefixed with its schema by default) - function that takes schema (`None` for 
default schema) and name of the enum and returns name of the module level variable for it. 
Used only if `module_level_enums` is turned on.
