...
def test_partial_index_preserved_during_enum_modification(connection: "Connection"):
    ...
    assert enum_values == sorted(
        new_enum_variants
    ), f"Enum values should be {sorted(new_enum_variants)}, got {enum_values}"
    ...
    with connection.begin_nested() as sp:
        try:
            ...
            sp.rollback()
            assert (
                "uq_user_username" in str(e) or "duplicate" in str(e).lower()
            ), f"Expected uniqueness violation, got: {e}"
...
def test_fallback_detection_of_partial_indexes_with_enum_expressions(connection: "Connection"):
    ...
    expected_indexes = {"idx_high_priority_tasks", "idx_urgent_tasks", "idx_active_tasks", "idx_medium_priority"}

    assert expected_indexes.issubset(index_names), (
        f"Expected indexes {expected_indexes} not found. " f"Found indexes: {index_names}"
    )
...
def test_non_partial_indexes_not_explicitly_dropped(connection: "Connection"):
    ...
    regular_index_oid = connection.execute(
        sqlalchemy.text("SELECT oid FROM pg_class WHERE relname = 'idx_items_status'")
    ).scalar()

    composite_index_oid = connection.execute(
        sqlalchemy.text("SELECT oid FROM pg_class WHERE relname = 'idx_items_status_created'")
    ).scalar()

    partial_index_oid = connection.execute(
        sqlalchemy.text("SELECT oid FROM pg_class WHERE relname = 'idx_items_published'")
    ).scalar()
    ...
    assert (
        regular_index_oid != regular_index_oid_after
    ), "Regular index will be rebuilt by ALTER COLUMN TYPE (different OID expected)"
    assert (
        composite_index_oid != composite_index_oid_after
    ), "Composite index will be rebuilt by ALTER COLUMN TYPE (different OID expected)"
    assert partial_index_oid != partial_index_oid_after, "Partial index will be rebuilt (different OID expected)"
    ...
    result = connection.execute(
        sqlalchemy.text("SELECT unnest(enum_range(NULL::item_status))::text as value ORDER BY value")
    )
    enum_values = [row.value for row in result]
